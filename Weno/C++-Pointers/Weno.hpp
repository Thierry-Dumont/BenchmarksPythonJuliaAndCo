...
public:
  ...
  Weno(int _size,double _L,double FluxParams[]): size(_size),h1(-1./(_L/size))
  {
    F=Flux(FluxParams);
    // 
    reconstructed=new double[2*size+8];
    InC=new double[size+4];
    work=new double[size+2];
  ...
  void operator()(double * __restrict__ In,double *__restrict__ Out)
  {
