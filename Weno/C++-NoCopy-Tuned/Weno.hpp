...
template <typename Flux, typename T>
inline
T WenoFluxKernel(Flux const& flux, T const* data)
{
    const auto rec = WenoLeftRightRecKernel(data);
    return flux(rec.first, rec.second);
}
...
public:
    ...
    {
        using details::WenoFluxKernel;

        std::size_t const size = data.size();
        Real const h1 = -1. / (length/size);

#ifndef USE_ORIGINAL_VERSION // Tuned version
        std::vector<Real> rec(size+1);

        // Left border
        rec[0] = WenoFluxKernel(flux, data[size-3], data[size-2], data[size-1], data[0], data[1], data[2]);
        rec[1] = WenoFluxKernel(flux, data[size-2], data[size-1], data[0], data[1], data[2], data[3]);
        rec[2] = WenoFluxKernel(flux, data[size-1], data[0], data[1], data[2], data[3], data[4]);


        // Center part
        for (std::size_t pos = 2; pos < size-3; ++pos)
            rec[pos+1] = WenoFluxKernel(flux, data[pos-2], data[pos-1], data[pos], data[pos+1], data[pos+2], data[pos+3]);

        // Right border
        rec[size-2] = WenoFluxKernel(flux, data[size-5], data[size-4], data[size-3], data[size-2], data[size-1], data[0]);
        rec[size-1] = WenoFluxKernel(flux, data[size-4], data[size-3], data[size-2], data[size-1], data[0], data[1]);
        rec[size] = WenoFluxKernel(flux, data[size-3], data[size-2], data[size-1], data[0], data[1], data[2]);

        for (std::size_t pos = 0; pos < size; ++pos)
            result[pos] = h1 * (rec[pos+1] - rec[pos]);
